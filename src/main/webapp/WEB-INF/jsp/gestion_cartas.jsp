							...
							<c:forEach items="${user.cartasPropias}" var="cartaPropia">
								<tr id="oRow${cartaPropia.id}">
									...
									<td class="filterable-cell">
										<button type="button" class="btn btn-xs spinner"
											onclick="decrSpinner(event)">-</button> <input
										id="qORow${cartaPropia.id}" type="text" name="quantity"
										class="cantidad-carta" value="${cartaPropia.cantidad}">
										<button type="button" class="btn btn-xs spinner"
											onclick="incrSpinner(event)">+</button>
									</td>
								...
								<input id="nameORow${cartaPropia.id}" type="hidden"
									value="${cartaPropia.carta.name}" name="cardsO[]"></input>
								<input id="edORow${cartaPropia.id}" type="hidden"
									value="${cartaPropia.carta.edicion}" name="cardsOE[]"></input>
								<input id="quantityORow${cartaPropia.id}" type="hidden"
									value="${cartaPropia.cantidad}" name="cardsOQ[]"></input>
								<input id="stateORow${cartaPropia.id}" type="hidden"
					...
					<table id="tab3" class="table table-striped">
						...
						<tbody>
							...
							<c:forEach items="${user.cartasBuscadas}" var="cartaBuscada">
								...
								<input id="nameSRow" ${cartasBuscada.id} type="hidden" value="1"
									name="cardsS[]"></input>
								<input id="edSRow${cartasBuscada.id}" type="hidden" value="1"
									name="cardsSE[]"></input>
							</c:forEach>
						</tbody>
					</table>
